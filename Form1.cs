    ...
    public partial class Form1 : Form
    {
        ...
        public void kayitlari_getir3()
        {
            ...
            for (int i = 0; i < Convert.ToInt32(toplamGun); i++)
            {
                ...
                DataGridViewColumn column = maliyet_dt.Columns[i + 8];
            ...
            for (int i = 0; i < personelkayıt_dt.Rows.Count; i++)
            {
                ...
                for (int k = 12 + Convert.ToInt32(toplamGun); k < 13 + Convert.ToInt32(toplamGun); k++)
                {
                ...
                for (int k = 13 + Convert.ToInt32(toplamGun); k < 14 + Convert.ToInt32(toplamGun); k++)
                {
                ...
                for (int k = 14 + Convert.ToInt32(toplamGun); k < 15 + Convert.ToInt32(toplamGun); k++)
                {
        ...
        private void sil_btn_Click(object sender, EventArgs e)
        {
            ...
            if (cevap == DialogResult.Yes)
            {
                ...
                baglanti1.Open();
                SqlCommand cmd = new SqlCommand("UPDATE personel_info SET name='" + ad_txtbx.Text + "', surname='" + soyad_txtbx.Text + "',reg_number='" + sicilno_txtbx.Text + "',job_id='" + statu_chckbx.SelectedValue + "',add_ind='" + ekgostege_txtbx.Text + "',state='" + false + "',note='" + not_txtbx.Text + "' WHERE reg_number='" + personelkayıt_dt.CurrentRow.Cells[3].Value.ToString() + "' ", baglanti1);//update işlemleri
                cmd.ExecuteNonQuery();
                kayitlari_getir();
                temizle();
            }
        ...
        private void yemekkayıtguncelle_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                SqlConnection connection = new SqlConnection("Data Source=.\\SQLEXPRESS;Integrated Security=SSPI;Initial Catalog=Daily_Eating");
                connection.Open();
                SqlCommand cmd = new SqlCommand("UPDATE personal_eat_food SET eat_piece='" + 1 + "', guest='" + (Convert.ToInt32(miktar2_nupdown.Value) - 1) + "' WHERE id='" + dataGridView4.CurrentRow.Cells[0].Value.ToString() + "' ", connection);//update işlemleri
                cmd.ExecuteNonQuery();
                MessageBox.Show("Güncellendi", "Bilgi");
                kayitlari_getir4();
            }
        ...
        private void yemekkayıtsil_btn_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection connection = new SqlConnection("Data Source=.\\SQLEXPRESS;Integrated Security=SSPI;Initial Catalog=Daily_Eating");
            connection.Open();
            SqlCommand cmd = new SqlCommand("DELETE personal_eat_food WHERE id='" + dataGridView4.CurrentRow.Cells[0].Value.ToString() + "' ", connection);//delete işlemi
            cmd.ExecuteNonQuery();
        ...
        private void passwordyenile_btn_Click(object sender, EventArgs e)
        {
            ...
            if (password1_txtbx.Text == password2_txtbx.Text)
            {
                SqlConnection connection = new SqlConnection("Data Source=.\\SQLEXPRESS;Integrated Security=SSPI;Initial Catalog=Daily_Eating");
                connection.Open();
                SqlCommand cmd = new SqlCommand("UPDATE admin_info SET username='" + username_txtbx.Text + "', password='" + password1_txtbx.Text + "' WHERE username='" + username_txtbx.Text+"' ", connection);//update işlemleri
                cmd.ExecuteNonQuery();
